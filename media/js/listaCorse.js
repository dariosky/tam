function moveToNext() {
	...
	if (next) {
		...
		$.scrollTo( next, 800, {offset: {top:-100} });
	}
}
...
$( function() {
		...
		$(".viaggioId").click( function(){		// ma lo mostro se seleziono qualche corsa
			...
			else $('#selActions').hide();
		});

		$('#linkUrl').click(function(){ $('#assoType').attr('value', 'link'); })
		$('#linkUrlBus').click(function(){ $('#assoType').attr('value', 'bus'); })
		$('#unlinkUrl').click(function(){ $('#assoType').attr('value', 'unlink'); })
