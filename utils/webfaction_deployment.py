    ...
    with cd(remote_path):
        if not exists("redis.conf"):
            if not exists("redis-%s.tar.gz" % redis_version):
                ...
                run(
                    'wget "http://download.redis.io/releases/redis-%s.tar.gz"'
                    % redis_version
                )
        ...
        for key, expected_value in keys.items():
            ...
            if not current_value:
                ...
                run(
                    r"sed -r -i 's/^({key}) .*$/\1 {expected_value}/' {file}".format(
                        key=key,
...
def redis_to_crontab(app_name):
    ...
    with fab_settings(warn_only=True):  # grep return errorcode when missing
        crontab = run(
            "crontab -l| grep {webapp_folder}/redis-server".format(
        ...
        if crontab.return_code != 0:
            run(
                '( crontab -l | grep -v -F "{cmd}" ; echo "{cmd}" ) | crontab -'.format(
...
def create_all_domains():
    ...
    for domain, subdomains in group_subdomains.items():
        if domain not in existing_domains or not all(
            ...
            print(
                "Creating domain {domain} with subdomains {sub}".format(
...
def create_all_apps():
    ...
    if main_app_name not in existing_apps:
        ...
        main_app = api.create_app(
            main_app_name, app_type="custom_websockets_app_with_port"
        )
    else:
        main_app = existing_apps[main_app_name]
    if settings.DEPLOYMENT["GUNICORN"]["PORT"] != main_app["port"]:
        print(
            "WARNING: Remember to change the interface server PORT to %s"
            % main_app["port"]
        )
        print(
            'Setting settings["DEPLOYMENT"]["GUNICORN"]["PORT"] = %s' % main_app["port"]
        )
    ...
    if "media" in webfaction_apps:
        ...
        if media_app_name not in existing_apps:
            print(
                "Creating media app {appname}: {mediafolder}".format(
                    appname=media_app_name,
                    mediafolder=media_folder,
                )
            )
            app = api.create_app(
                media_app_name, app_type="symlink70", extra_info=media_folder
            )
    if "static" in webfaction_apps:
        ...
        if static_app_name not in existing_apps:
            print(
                "Creating staic app {appname}: {staticfolder}".format(
                    appname=static_app_name,
                    staticfolder=static_folder,
                )
            )
            app = api.create_app(
                static_app_name, app_type="symlink70", extra_info=static_folder
            )
...
def create_all_websites():
    ...
    if not api.website_exists(main_app_name, existing_websites):
        ...
        api.create_website(
            main_app_name, websites_ip, False, [settings.WEBHOST], "", apps
        )

    hosts = get_site_hostnames()
    if (
        not api.website_exists(webfaction_apps["media"], existing_websites)
        and hosts["media"]
    ):
    ...
    if (
        not api.website_exists(webfaction_apps["static"], existing_websites)
        and hosts["static"]
    ):
