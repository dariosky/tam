...
def getBackupInfo(doCleanup=False):
    ...
    if doCleanup and len(backups) > backcount:
        for backup in backups[: -backcount + 1]:
            logging.debug(
                "Cancello il backup: %s" % os.path.basename(backup["filename"])
            )
    ...
    for backup in backupInfo["backups"]:
        if backup["date"].timetuple()[:5] == dataScelta.timetuple()[:5]:
            ...
            response = HttpResponse(
                responseFile.read(), content_type="application/octet-stream"
            )  # 'application/gzip'
            response[
                "Content-Disposition"
            ] = 'attachment; filename="%s"' % os.path.basename(backup_filename)
            return response
    else:
...
def doBackupSqlite(targetFile, sourceFile):
    ...
    f_out = gzip.GzipFile(
        dbname, fileobj=real_out
    )  # scrivo sul file Gzip il contenuto si chiama sempre tam.db3
...
def doBackupPostgre(
    targetFile, dbName, host=None, port=None, username=None, password=None
):
...
def doBackup(user):
    ...
    if dbType == "django.db.backends.sqlite3":
        doBackupSqlite(
            targetFile=backupPath + ".db3.gz", sourceFile=backupInfo["dbname"]
        )
...
def backup(request, template_name="utils/backup.html"):
    ...
    if "backup" in request.POST:
        ...
        messages.success(
            request, "Backup del database avviato... sarà pronto tra poco."
        )
        return HttpResponseRedirect(reverse("tamBackup"))
