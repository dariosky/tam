...
def styleCopy(style=None, **kwargs):
    ...
    if style:
        ...
        if (style.alignment.horz, style.alignment.vert) == (
            ...
            xlwt.Alignment.HORZ_RIGHT, xlwt.Alignment.VERT_CENTER):
            proplist.append("right")
        if style.alignment.wrap == xlwt.Alignment.WRAP_AT_RIGHT:
            proplist.append("wrap")
        if style.borders.left > 0:
            proplist.append(
                "border%s" % style.borders.left)  # tutti i bordi hanno lo stesso spessore
...
def styleFromField(field, stile=sty(), **kwargs):
    ...
    if stile.num_format_str == "general":
        if isinstance(field,
                      datetime.datetime) and field.hour == field.min == field.second == field.microsecond == 0:
            field = field.date()
...
def myWrite(sheet, y, x, field, style=sty()):
    ...
    xlsColWidth[x] = max(xlsColWidth.get(x, 3000),
                         310 * field_length)  # default width and computed column width
...
class Sql2xls(object):
    ...
    def __init__(self, doc, con=None, query=None, processRowCallback=None, additionalHeaders=None,
                 **kwargs):
        self.sheetCount = 0
        self.query = query
        self.doc = doc
    ...
    def run(self):
        ...
        for results in self.enumerateTables(self):
            ...
            if self.imageFields and self.sheetCount < len(self.imageFields):
                ...
                from PIL import \
                    Image as PILImage  # used to write and convert embedded images @UnresolvedImport
            else:
                imageField = None
                    ...
                    if os.path.isfile(self.imagePath):
                        ...
                        try:
                            ...
                            xlsRowHeight(foglio, y, int(
                                newsize[
                                    1] * 13))  # l'altezza della riga è la dimensione dell'immagini in pixel * 13
