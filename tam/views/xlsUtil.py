...
def styleCopy(style=None, **kwargs):
    ...
    if style:
        if style.font.bold:
            proplist.append("b")
        if (style.alignment.horz, style.alignment.vert) == (
            xlwt.Alignment.HORZ_CENTER,
            xlwt.Alignment.VERT_CENTER,
        ):
            proplist.append("center")
        elif (style.alignment.horz, style.alignment.vert) == (
        ...
        if style.borders.left > 0:
            proplist.append(
                "border%s" % style.borders.left
            )  # tutti i bordi hanno lo stesso spessore
...
def sty(
    propList=None, num_format="general", background_color=None, foreground_color=None
):
...
def styleFromField(field, stile=sty(), **kwargs):
    ...
    if stile.num_format_str == "general":
        if (
            isinstance(field, datetime.datetime)
            and field.hour == field.min == field.second == field.microsecond == 0
        ):
            field = field.date()
...
def myWrite(sheet, y, x, field, style=sty()):
    if isinstance(field, Decimal):
        ...
        field = ""
    try:
        if hasattr(field, "decode") and not isinstance(field, str):
    ...
    xlsColWidth[x] = max(
        xlsColWidth.get(x, 3000), 310 * field_length
    )  # default width and computed column width
...
def row2dict(row, desc):
    ...
    for i, field in enumerate(desc):
        if i >= len(row):
            break
        if hasattr(row[i], "strip") and not isinstance(row[i], str):
            result[field] = str(row[i].strip(), errors="ignore")
        else:
            result[field] = row[i]
    return result
...
def boxFit(dimensions, maxX, maxY=None):
    ...
    if x <= maxX and y <= maxY:
        return (x, y)  # lesser or equal, no need to resize
    aspect = float(x) / y
    rx, ry = x, y
    if rx > maxX:
        rx = maxX
...
class Sql2xls(object):
    ...
    def enumerateSQLTables(self):
        ...
        self.imageFields = [
            g.strip() for g in re.findall(r"--imageField:(.*?)\n", query)
        ]  # field name wich will go through mimagoFilename
        cur = self.con.cursor()
    ...
    def run(self):
        ...
        for results in self.enumerateTables(self):
            ...
            if self.imageFields and self.sheetCount < len(self.imageFields):
                imageField, imageMaxX, imageMaxY = splitImageFormat(
                    self.imageFields[self.sheetCount]
                )
                from PIL import (
                    Image as PILImage,
                )  # used to write and convert embedded images @UnresolvedImport
            else:
                imageField = None
            ...
            for row in results:
                ...
                if imageField:
                    if os.path.isfile(self.imagePath):
                        img = PILImage.open(open(self.imagePath, "rb"))
                        img = img.convert("RGB")
                        newsize = boxFit(
                            (img.size[0], img.size[1]), imageMaxX, imageMaxY
                        )  # make an Image a little bigger
                        img = img.resize(newsize, PILImage.ANTIALIAS)  # resize inplace
                        try:
                            tmpImage = open(tmpImg, "wb")
                            tmpImgToRemove = True
                            img.save(tmpImage, "BMP", quality=85)  # PNG is also valid
                            tmpImage.close()
                            xlsRowHeight(
                                foglio, y, int(newsize[1] * 13)
                            )  # l'altezza della riga è la dimensione dell'immagini in pixel * 13
