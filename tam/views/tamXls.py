    ...
    for record in manager:
        ...
        for field in fields.keys():
            if "." in field:
                ...
                if field == "incassato_albergo" and getattr(
                    settings, "TAM_EXPORT_EXCEL_FINEMESE_LORDO", False
                ):
                    # a request to put contofinemese as lordo
                    related = (
                        record.__getattribute__("prezzo")
                        if record.__getattribute__(field)
                        else 0
                    )
                else:
...
def xlsResponse(request, querySet):
    ...
    if numViaggi > settings.MAX_XLS_ROWS:
        messages.error(
            request,
            "Non puoi esportare in Excel più di %d viaggi contemporaneamente."
            % settings.MAX_XLS_ROWS,
        )
