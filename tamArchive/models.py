...
class ViaggioArchive(models.Model):
    ...
    path = models.TextField("Percorso effettuato",
                            blank=True)  # il percorso come stringa HTML con i tempi
    ...
    flag_richiesto = models.BooleanField("Conducente richiesto",
                                         default=False)  # True quando il conducente è fissato

    cliente = models.CharField("A", max_length=40,
                               null=True)  # cliente o passeggero, come testo - se null è un privato non specificato

    prezzo = models.DecimalField(max_digits=9, decimal_places=2, editable=False,
                                 default=0)  # prezzo lordo al cliente
