...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        for utentePrenotazioni in orm.UtentePrenotazioni.objects.all():
            utentePrenotazioni.clienti.clear()
            utentePrenotazioni.clienti.add(utentePrenotazioni.cliente)
            print(
                "Aggiungo il cliente %s all'utente %s"
                % (utentePrenotazioni.cliente.nome, utentePrenotazioni.user.username)
            )
            utentePrenotazioni.save()

    def backwards(self, orm):
        raise RuntimeError(
            "Non posso annullare la migrazione: non saprei che utente scegliere"
        )
    ...
    models = {
        ...
        "prenotazioni.utenteprenotazioni": {
            ...
            "cliente": (
                ...
                [],
                {
                    "related_name": "'unicoUtentePrenotazione'",
                    "to": "orm['tam.Cliente']",
                },
            ),
            "clienti": (
                "django.db.models.fields.related.ManyToManyField",
                [],
                {"to": "orm['tam.Cliente']", "symmetrical": "False"},
            ),
            "email": (
                "django.db.models.fields.EmailField",
                [],
