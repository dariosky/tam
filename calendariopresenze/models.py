...
class Calendar(models.Model):
    conducente = models.ForeignKey(
        "tam.Conducente", on_delete=CASCADE, related_name="presenze"
    )
    ...
    available = models.BooleanField(
        default=False
    )  # When this is True the conducente is still available
    value = (
        models.IntegerField()
    )  # value is the minutes of duration or other kind of score
    ...
    def __str__(self):
        caldesc = settings.CALENDAR_DESC[self.type]
        result = "{conducente}. {cal_name} {date_start} ".format(
        ...
        if "display_as" in caldesc:
            ...
            result += "{duration} [tipo {type}]".format(
                duration=self.pretty_duration(), type=self.type
            )
        return result + (" but available" if self.available else "")
