...
class CalendarManage(AjaxableResponseMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "mediabundleJS": ("tamUI",),  # I need the datepicker
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not context["can_edit"]:
            return self.render_to_response(
                dict(
                    message="Non hai permessi per modificare le presenze.",
                    status=401,
        ...
        if action == "new":
            ...
            if "time_from" in request.POST:
                ...
                try:
                    ...
                    return self.render_to_response(
                        dict(
                            message="Ora iniziale non valida.",
                            status=400,
                            redirect_url="calendariopresenze-manage",
                        )
                    )
            ...
            if request.is_ajax():
                ...
                return HttpResponseRedirect(
                    reverse("calendariopresenze-manage")
                    + "?day="
            ...
            if not request.user.has_perm("calendariopresenze.delete_calendar"):
                return self.render_to_response(
                    dict(
                        message="Non hai permessi per cancellare le presenze.",
                        status=401,
                        redirect_url="calendariopresenze-manage",
                    )
                )
            try:
                ...
                return self.render_to_response(
                    dict(
                        message="Il calendario indicato non esiste più.",
                        status=400,
                        redirect_url="calendariopresenze-manage",
                    )
                )
            ...
            logAction(
                ...
                description="Presenze: cancellato {name} {calendar}".format(
                    name=caldesc["name"], calendar=calendar
                ),
            )
            calendar.delete()
            ...
            if not request.user.has_perm("calendariopresenze.toggle_calendarvalue"):
                return self.render_to_response(
                    dict(
                        message="Non hai permessi per modificare il valore dei calendari.",
                        status=401,
                        redirect_url="calendariopresenze-manage",
                    )
                )
            ...
            if "toggle" not in caldesc:
                return self.render_to_response(
                    dict(
                        message="Valore del calendario non modificabile.",
                        status=400,
                        redirect_url="calendariopresenze-manage",
                    )
                )
            ...
            if request.is_ajax():
                ...
                context.update(
                    dict(
                        element=calendar, calDesc=settings.CALENDAR_DESC[calendar.type]
                    )
                )
                return HttpResponse(row_template.render(context, request), status=201)
            else:
...
class CalendarRank(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for key in calendars:
            ...
            ranks = []
            calendar_filter_query = Q(
                attivo=True, presenze__type=key
            )  # calendar do not reset by default
            if caldesc.get("rank_start"):
                ...
                calendar_filter_query = calendar_filter_query & Q(
                    ...
                    presenze__date_start__lt=caldesc["calendar_reset_date_end"],
                )
            ...
            for conducente in conducenti_da_annotare.annotate(
                ...
                ranks.append(
                    {
                        "name": conducente.nome,
                        "conducente_id": conducente.id,
                        "value": value,
                    }
                )
            caldesc["rank"] = ranks
        context["calendars"] = calendars
        context.update(
            dict(year=year, previous_year=previous_year, next_year=next_year)
        )
        return context
...
class CalendarByConducente(CalendarRank):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "calendar_reset_date" in caldesc:
            calendar_query = calendar_query & Q(
                date_start__gte=caldesc["calendar_reset_date"],
